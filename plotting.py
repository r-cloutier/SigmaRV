...
cbticklabels[np.array([0,7,9,16,18,25,27])] = ['0.3','1','3','10','30','100','300']
...
def plot_grid(pltt=True, label=False, vmin=.3, vmax=3e2):
    ...
    fig.subplots_adjust(left=.095, bottom=.2, top=.95, right=.99, hspace=.1, wspace=.1)
    if label:
...
def reduce_dimension(arr, axes, sigma=.7):
